...
if [[ -f /etc/yudhynet/github/api && -f /etc/yudhynet/github/email && /etc/yudhynet/github/username ]]; then
   rec="OK"
else
    mkdir /etc/yudhynet/github > /dev/null 2>&1
fi
...
sleep 2
echo "$EMAIL1" > /etc/yudhynet/github/email
echo "$USERNAME1" > /etc/yudhynet/github/username
echo "$API1" > /etc/yudhynet/github/api
echo "ON" > /etc/yudhynet/github/gitstat
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
function resetipvps(){
clear
rm -f /etc/yudhynet/github/email
rm -f /etc/yudhynet/github/username
rm -f /etc/yudhynet/github/api
rm -f /etc/yudhynet/github/gitstat
echo "OFF" > /etc/yudhynet/github/gitstat
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
Isadmin=$(curl -sS https://raw.githubusercontent.com/bakulssh/permission/main/ipmini | grep $MYIP | awk '{print $5}')
if [ "$Isadmin" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
...
if [ -f "$GITREQ" ]; then
    ...
    mkdir /etc/yudhynet/github
    touch /etc/yudhynet/github/gitstat
    echo "OFF" > /etc/yudhynet/github/gitstat
fi

stst1=$(cat /etc/yudhynet/github/gitstat)
if [ "$stst1" = "OFF" ]; then
clear
echo -e "$COLOR1┌─────────────────────────────────────────────────┐${NC}"
echo -e "$COLOR1 ${NC} ${COLBG1}               ${WH}• REGISTER IPVPS •              ${NC} $COLOR1 $NC"
...
stst=$(cat /etc/yudhynet/github/gitstat)
if [ "$stst" = "ON" ]; then
APIOK="CEK API"
rex="viewapi"
else
